 ...
 * 身体成分分析仪usb通信
 */
class BodyUsbSerialPort(private val activity: Activity) : AbsSerialPort(activity) {


    private var deviceId: String = ""
    private var portNum: Int = 0
    private var baudRate: Int = 19200//9600

    private var usbIoManager: SerialInputOutputManager? = null
    private var usbSerialPort: UsbSerialPort? = null
    private var connected = false
    private val TAG = "BODYSERIALPORT"
    override fun port(): String {
       return deviceId
    }

    override fun connect(vararg params: String) {
        Log.i(TAG, "TAG connect params:$params")
        if (connected) {
            return
        }
        ...
        try {
            usbSerialPort?.apply {
                ...
                Log.i(javaClass.simpleName, "TAG connected")
    ...
    override fun sendData(data: String) {
        if (!connected) {
            Log.d(javaClass.simpleName, "TAG not connected")
            return
        }
        try {
            usbSerialPort?.write(
    ...
    override fun registerDataListener() {
        usbIoManager?.apply {
            listener = this@BodyUsbSerialPort
        }
    }
