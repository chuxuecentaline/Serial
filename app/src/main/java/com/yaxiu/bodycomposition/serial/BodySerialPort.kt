 ...
 * @email
 */
class BodySerialPort : ISerialPort, OnNormalDataListener {
    ...
    override fun sendData(data: String) {
        if (isConnected()) {
            ...
            try {
                val instance = NormalSerial.instance()
                val declaredField = instance.javaClass.getDeclaredField("mBaseSerial")
                declaredField.isAccessible=true
                val mBaseSerial = declaredField.get(instance)
                if (mBaseSerial is BaseSerial) {
                    mBaseSerial.sendTxt(data)
                }
    ...
    override fun sendData(data: ByteArray) {
        if (isConnected()) {
            ...
            try {
                ...
                val mBaseSerial = declaredField.get(instance)
                if (mBaseSerial is BaseSerial) {
                    mBaseSerial.sendByteArray(data)
                }
    ...
    override fun normalDataBack(hexData: String) {
        Log.i(javaClass.simpleName, "TAG normalDataBack:$hexData")
        mHexData = hexData
    }

    override fun equals(other: Any?): Boolean {
        if (other is BodySerialPort) {
